...
# Make a fresh hash table with the given number of bins 'size', containing no elements.
...
def add(ht: HashTable, word: str, line: int) -> None:
    ...
    while node is not None:
        if node.first.word == word:
            ...
            return
        node = node.rest
    wl = WordLines(word, IntList(line, None))
